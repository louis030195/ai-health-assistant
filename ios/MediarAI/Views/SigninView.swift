    ...
    Task {
      do {
        ...
        url = try client.auth.getOAuthSignInURL(
          provider: Provider.google, redirectTo: URL(string: "mediar://auth-callback")!)
        print("url: \(url)")
  ...
  func loginWithApple() {
    Task {
      do {
        ...
        url = try client.auth.getOAuthSignInURL(
          provider: Provider.apple, redirectTo: URL(string: "mediar://auth-callback")!)
        url = URL(string: url.absoluteString.replacingOccurrences(of: "+", with: "%20"))!
        print("url: \(url)")
        model.showSafari = true
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      VStack(alignment: .leading, spacing: 32) {
        ...
        VStack(alignment: .leading, spacing: 16) {
          InputFieldView(
            label: "Email", value: $email, placeholder: "john@mediar.ai", isSecure: false)
          InputFieldView(
            label: "Password", value: $password, placeholder: "*******", isSecure: true)
        }
        .frame(maxWidth: .infinity)
